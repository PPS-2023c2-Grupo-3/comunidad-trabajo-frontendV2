...
const ListadoOfertas = () => {
  ...
  useEffect(() => {
    const primerLlamado = async () => {
      try {
        const response = await getOfertas(0, 6, "", "id", 5);
  ...
  const traerOfertas = async (e) => {
    ...
    try {
      const response = await getOfertas(0, 6, ofertaValue, "id", 5);
      setOfertas(response.ofertas.rows);
      setCantPaginas(response.totalPaginas);
    } catch (err) {
      console.log(err);
  ...
  const cambiarPagina = async (e, p) => {
    try {
      const response = await getOfertas(p - 1, 6, busquedaActual, "id", 5);
      setOfertas(response.ofertas.rows);
      setPagina(p);
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      ...
      {ofertas.length === 0 ? (
        ...
        <ListaOfertasFinalizadas ofertas={ofertas} />
      )}
      <Pagination
        color="primary"
        count={cantPaginas}
